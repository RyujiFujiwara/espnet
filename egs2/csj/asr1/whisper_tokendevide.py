...
## SETTING
_scoredir = "/mnt/kiso-qnap/fujiwara/B4/main/espnet/egs2/csj/asr1/exp/asr_train_asr_whisper_large_decselfatten_finetune_raw_ja_whisper_multilingual_sp/decode_asr_whisper_noctc_primtoken_asr_model_valid.acc.ave/eval1/score_wer"
##
...
with open(input_ref, "r", encoding="utf-8") as ref_file, \
    ...
    for ref_line, hyp_line in zip(ref_file, hyp_file):
        ...
        ref_token_devide = [tokenizer.tokens2text([token]) for token in ref_token_devide]
        ...
        hyp_token_devide = [tokenizer.tokens2text([token]) for token in hyp_token_devide]
